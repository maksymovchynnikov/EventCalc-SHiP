...
class Grids:
    ...
    def interpolate(self, timing=False):
        ...
        # guard against invalid interpolated values
        mask = (~np.isfinite(self.max_energy)) | (self.max_energy < self.m)
        self.max_energy[mask] = 1.01 * self.m
        ...
        # guard against bad interpolation
        mask = (self.interpolated_values < 0) | np.isnan(self.interpolated_values)
        self.interpolated_values[mask] = 0.0
    ...
    def get_instant_decay_momentum(self, timing=False):
        ...
        self.momentum = np.column_stack((px, py, pz, self.r_energy))
